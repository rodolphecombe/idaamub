...
local commands = {
	...
	transmute_ground = function(operation, unit)
		...
		loti.item.on_the_ground.transmute(operation.number, operation.sort, unit.x, unit.y)
	end,
	sell_ground = function(operation, unit)
		-- Sell an item lying on the ground
		loti.item.on_the_ground.sell(operation.number, operation.sort, unit.x, unit.y)
	end,
